...
def test_platform_validation_tasks_exist_in_dependency_order() -> None:
    dag_bag = load_dag_bag()
    dag = dag_bag.dags.get(DAG_ID)

    assert dag is not None, f"{DAG_ID} DAG was not loaded"

    batch_status_task = dag.get_task(
        "check_batch_pipeline_status"
    )
    streaming_status_task = dag.get_task(
        "check_streaming_pipeline_status"
    )
    cloud_ready_task = dag.get_task(
        "check_cloud_platform_ready"
    )
    ...
    assert batch_status_task.downstream_task_ids == {
        "check_cloud_platform_ready"
    }

    assert streaming_status_task.downstream_task_ids == {
        "check_cloud_platform_ready"
    }

    assert cloud_ready_task.upstream_task_ids == {
        "check_batch_pipeline_status",
        "check_streaming_pipeline_status",
    }

    assert cloud_ready_task.downstream_task_ids == {
        "generate_redshift_execution_summary"
    }
    ...
    assert orchestration_summary_task.downstream_task_ids == {
        "end"
    }
